...
import { MdSearch, MdEdit, MdDelete, MdClose, MdPerson } from 'react-icons/md';
...
function EditModal({ author, onClose, onSave }) {
  const [form, setForm] = useState({ name: author.name || '', email: author.email || '', phone: author.phone || '', description: author.description || '' });
  ...
  const handleSubmit = e => {
    e.preventDefault();
    setSaving(true);
    axiosInstance.put(`/update-author/${author._id}`, form)
      .then(res => {
        if (res.data.success) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 px-4">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-md p-6">
        <div className="flex items-center justify-between mb-5">
          <h2 className="text-base font-bold text-gray-800">Edit Author</h2>
...
function AddAuthors() {
  ...
  const handleAdd = e => {
    e.preventDefault();
    setAdding(true);
    axiosInstance.post('/add-authors', form)
      .then(res => {
        if (res.data.success) {
          toast.success('Author added!');
          setAuthors(prev => [...prev, { ...form, _id: res.data.id || Date.now().toString() }]);
          setForm(EMPTY_FORM);
        }
      })
      .catch(() => toast.error('Failed to add author'))
  ...
  return (
    <div className="p-6 lg:p-8">
      <div className="mb-6">
        <h1 className="text-xl font-bold text-gray-800">Authors</h1>
        <p className="text-sm text-gray-400 mt-0.5">Add and manage book authors</p>
      </div>
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
            <div className="flex items-center gap-2 mb-5">
              <div className="w-8 h-8 rounded-lg bg-green-50 flex items-center justify-center">
                <MdPerson size={17} className="text-green-600" />
              </div>
              <h2 className="text-sm font-bold text-gray-700">Add New Author</h2>
            </div>
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
            <div className="px-5 py-4 border-b border-gray-100 flex flex-col sm:flex-row sm:items-center gap-3">
              <p className="text-sm font-bold text-gray-700 flex-1">
                All Authors <span className="text-gray-400 font-normal ml-1">({authors.length})</span>
              </p>
            ...
            {loading ? (
              ...
              <div className="py-14 text-center">
                <MdPerson size={36} className="text-gray-200 mx-auto mb-2" />
                <p className="text-sm text-gray-400">{search ? 'No authors match your search.' : 'No authors yet.'}</p>
              </div>
            ) : (
              <div className="divide-y divide-gray-50 max-h-[calc(100vh-14rem)] overflow-y-auto">
                {filtered.map(a => (
                  <div key={a._id} className="px-5 py-3.5 flex items-start gap-3 hover:bg-gray-50/50 transition-colors">
                    <div className="w-8 h-8 rounded-full bg-violet-100 text-violet-600 text-xs font-bold flex items-center justify-center flex-shrink-0 mt-0.5 select-none">
      ...
      {editing && (
        <EditModal author={editing} onClose={() => setEditing(null)} onSave={handleSave} />
      )}
    </div>
  );
}
